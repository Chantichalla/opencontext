# Synapse Analysis & Context Architecture Report

## 1. Synapse PRD Analysis
**Synapse** is proposed as a shared AI memory layer for development teams using VS Code. Its primary goal is to solve the "isolated AI problem" where individual developers' AI coding sessions lack awareness of team-wide architectural decisions and current module ownership.
...
## 3. Deep Research: Ways to Build the Synapse Tool
Based on the `@[/deep-research]` workflow, here is a comprehensive analysis of how to technically build the Synapse tool, drawing from current web paradigms regarding AI architecture extensibility.

### Option A: The Continue.dev Custom Context Provider (Recommended path)
As identified in your PRD, intercepting prompts for tools like GitHub Copilot is functionally undocumented and highly fragile. The most robust way to build Synapse is by hooking into **Continue.dev**, an open-source AI assistant framework.
...
2. **The Flow:** Synapse's local VS Code extension can spin up an internal HTTP server (or the Continue config can point directly to the Synapse local cache). When the user types a prompt, Continue.dev sends a request to this HTTP endpoint with the user's active file information.
3. **Response:** The Synapse endpoint reads the local context `JSON`, filters out the non-relevant modules based on the file path, and returns the curated string to Continue.dev, which automatically prepends it to the LLM prompt.
...
- You could build the Synapse Sync Server as an **MCP Server**. 
- Any MCP-compatible VS Code extension (like Claude for VS Code, or potentially Cursor in the future) could natively connect to your Synapse MCP Server to retrieve the "Decision Memory" as explicit tools or context chunks.
...
3. **Conflict Resolution:** Since AI context is eventually consistent, a simple "Last-Write-Wins" policy is acceptable for V1. When a developer triggers `Synapse: Save Decision`, the server updates the SQLite DB and broadcasts an invalidation payload to all connected WebSocket clients, forcing their local caches to update within milliseconds.

### Conclusion
Building Synapse is highly feasible today. The key architectural pivot is relying on **open ecosystems** (Continue.dev context providers or the Model Context Protocol) rather than trying to reverse-engineer closed systems like GitHub Copilot's prompt ingestion pipeline.
